...
// CORS — open for now; restrict to specific domains after testing
app.use(cors({
  origin: true,   // allow all origins (tighten to specific domains in production)
...
app.get('/', (req, res) => {
  res.json({
    status:  'ok',
    service: 'CBE Mark Sheet Payment Backend',
...
// Ping endpoint — frontend calls this to wake server before payment
...
// ── Link sessionId → Firebase uid ─────────────────────────────────────────────
// Called after account creation so webhook can find school by uid
app.post('/api/stk-push/link', async (req, res) => {
  const { sessionId, userId } = req.body;
  if (!sessionId || !userId) return res.status(400).json({ error: 'sessionId and userId required' });

  try {
    if (db) {
      const sessSnap = await db.collection('payments').doc(sessionId).get();
      if (sessSnap.exists) {
        const data = sessSnap.data();
        // Copy payment doc under the real Firebase uid
        await db.collection('payments').doc(userId).set({ ...data, userId }, { merge: true });
        // Update session doc to record the uid
        await db.collection('payments').doc(sessionId).set({ userId }, { merge: true });
        console.log(`[link] Session ${sessionId} → uid ${userId}`);
      }
    }
